...
class SoftmaxMath:
    """Math helpers shared by linear and hidden-layer softmax classifiers."""

    @staticmethod
    def softmax(logits: np.ndarray) -> np.ndarray:
        """Compute row-wise softmax with max-shift for numerical stability."""
        # Subtracting row max keeps exponentials in a safe floating-point range.
        exp_logits = np.exp(logits - np.max(logits, axis=1, keepdims=True))
        return exp_logits / np.sum(exp_logits, axis=1, keepdims=True)

    @staticmethod
    def cross_entropy(probs: np.ndarray, y_true: np.ndarray) -> float:
        """Return average multiclass cross-entropy for one-hot targets."""
...
class SoftmaxRegressionClassifier:
    ...
    def initialize(self, seed: int) -> None:
        """Initialize parameters with small random weights and zero bias."""
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        """Return argmax class predictions for each sample."""
        probs = self.predict_proba(X)
        return np.argmax(probs, axis=1)

    def evaluate(self, X: np.ndarray, y: np.ndarray) -> Tuple[float, float]:
        """Return cross-entropy loss and accuracy on a dataset."""
        probs = self.predict_proba(X)
...
class HiddenLayerClassifier:
    """Single-hidden-layer tanh network with softmax output."""
    ...
    def initialize(self, seed: int) -> None:
        """Initialize network weights with scaled random values and zero bias."""
        rng = np.random.default_rng(seed)
    ...
    def forward(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Run forward pass and return (pre-activation, hidden, logits)."""
